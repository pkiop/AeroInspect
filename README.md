...
**한국어 점검 서술(점검 개요·종합 의견)** 을 자동 생성하는 멀티에이전트 데모입니다.
...
flowchart LR
    ...
    X -- "플래그/ESCALATED 조정" --> R["ReportAgent<br/>(한국어 서술 생성)"]
    R --> D["점검 개요 · 종합 의견<br/>(UI 카드 표시)"]
```
...
- **ReportAgent**: Gemini 구조화 출력으로 한국어 점검 개요·종합 의견 서술 생성
...
- **실행하지 않아도 데모는 동작합니다** — 스토어가 없으면 GroundingAgent가 자동으로
  카탈로그 전문을 프롬프트에 주입하는 **롱컨텍스트 폴백**으로 전환합니다(UI에 `via_fallback` 표시).
...
모두 가상으로 작성된 것이며, 산출되는 점검 결과는 **실제 항공기 감항성 판단에 사용할 수 없습니다**.
